...
def _fractal_dimension(prices: np.ndarray, window: int = 30) -> float:
    ...
    High FDI (> 1.55) mathematically confirms a mean-reverting (choppy) environment,
...
def _ewmv(returns: np.ndarray, alpha: float = 0.15) -> float:
    ...
    allowing the bot to detect sudden micro-volatility spikes instantly.
    """
    if len(returns) < 10:
        return 0.001
...
def _rolling_std(prices: np.ndarray, period: int = 15) -> float:
    """Calculates standard deviation of recent returns for Z-score barrier evasion."""
    if len(prices) < period + 1:
        return 0.001
    returns = np.diff(prices[-period-1:])
    return np.std(returns) + 1e-9
...
class AccumulatorStrategy:
    ...
    def should_exit(self, ticks: List[float], current_spot: float, high_barrier: float, low_barrier: float) -> bool:
        """
        ...
        # Determine the immediate tick velocity (Standard Deviation)
        current_std = _rolling_std(prices, period=15)
        ...
        # Probability Math: How many standard deviations away is the danger zone?
        z_score = closest_barrier_dist / current_std

        # EPSILON (Risk Tolerance):
        # If the barrier is closer than 2.5 standard deviations, the probability
...
def check_exit_condition(ticks: List[float], current_spot: float, high_barrier: float, low_barrier: float) -> bool:
    """Calculates live probability density to trigger an emergency exit. Returns True to Sell."""
    return ACCUMULATOR_ENGINE.should_exit(ticks, current_spot, high_barrier, low_barrier)
